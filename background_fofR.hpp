...
double R_dot_RungeKutta(const double, const double, const double,	const double,	double const * const params, const int fofR_type);
...
inline double dot_R_initial_fR(const double a, const double H, const double fourpiG, const cosmology cosmo, double const * const params, const int type)
{
	// Corresponds to background as computed using the modified Friedmann equation:
	// return R_dot_RungeKutta(a, H, R_initial_fR(a, fourpiG, cosmo), 2.*fourpiG*a*a*rho(a, cosmo), params, type);
...
double R_dot_RungeKutta(const double a,
												...
												const int fofR_type)
{
	double frr = FRR(R, params, fofR_type, 210);
	double fr = FR(R, params, fofR_type, 211);
	double denom = 3. * frr * H;

	if(denom)
	{
		double rdot = (eightpiG_rho_a2 - 3.*H*H*(1. + fr) + 0.5*(fr * R - F(R, params, fofR_type, 212))*a*a) / denom;
		return rdot;
	}
	else
	{
...
{
	double res;
	double f = F(R, params, fofR_type, 310);
	double fr = FR(R, params, fofR_type, 311);
	double frr = FRR(R, params, fofR_type, 312);
	double frrr = FRRR(R, params, fofR_type, 313);
									   ...
									   const int fofR_type)
{
	...
	R1 = R_dot_RungeKutta(a, H, R, 3. * Hconf(a, fourpiG, cosmo) * Hconf(a, fourpiG, cosmo), params, fofR_type);

	a2 = a_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau);
	H2 = H_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau);
	R2 = R_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau, 3. * Hconf(a + 0.5 * a1 * dtau, fourpiG, cosmo) * Hconf(a + 0.5 * a1 * dtau, fourpiG, cosmo), params, fofR_type);

	a3 = a_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau);
	H3 = H_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau);
	R3 = R_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau, 3. * Hconf(a + 0.5 * a2 * dtau, fourpiG, cosmo) * Hconf(a + 0.5 * a2 * dtau, fourpiG, cosmo), params, fofR_type);

	a4 = a_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau);
	H4 = H_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau);
	R4 = R_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau, 3. * Hconf(a + a3 * dtau, fourpiG, cosmo) * Hconf(a + a3 * dtau, fourpiG, cosmo), params, fofR_type);
									   ...
									   const int fofR_type)
{
	...
	a1 = a_dot_RungeKutta(a, H);
	H1 = H_dot_RungeKutta(a, H, R);
	R1 = R_dot_RungeKutta(a, H, R, 3. * Hconf(a, fourpiG, cosmo, T00_hom) * Hconf(a, fourpiG, cosmo, T00_hom), params, fofR_type);

	a2 = a_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau);
	H2 = H_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau);
	R2 = R_dot_RungeKutta(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau, 3. * Hconf(a + 0.5 * a1 * dtau, fourpiG, cosmo, T00_hom) * Hconf(a + 0.5 * a1 * dtau, fourpiG, cosmo, T00_hom), params, fofR_type);

	a3 = a_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau);
	H3 = H_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau);
	R3 = R_dot_RungeKutta(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau, 3. * Hconf(a + 0.5 * a2 * dtau, fourpiG, cosmo, T00_hom) * Hconf(a + 0.5 * a2 * dtau, fourpiG, cosmo, T00_hom), params, fofR_type);

	a4 = a_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau);
	H4 = H_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau);
	R4 = R_dot_RungeKutta(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau, 3. * Hconf(a + a3 * dtau, fourpiG, cosmo, T00_hom) * Hconf(a + a3 * dtau, fourpiG, cosmo, T00_hom), params, fofR_type);
									 ...
									 const int fofR_type)
{
	...
	R1 = dtau * R_dot_RungeKutta(a, H, R, 3. * Hconf(a, fourpiG, cosmo) * Hconf(a, fourpiG, cosmo), params, fofR_type);

	a2 = dtau * a_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1);
	H2 = dtau * H_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1, R + 0.25 * R1);
	R2 = dtau * R_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1, R + 0.25 * R1, 3. * Hconf(a + 0.25 * a1, fourpiG, cosmo) * Hconf(a + 0.25 * a1, fourpiG, cosmo), params, fofR_type);
	...
	R3 = dtau * R_dot_RungeKutta(a + 3./32. * a1 + 9./32. * a2, H + 3./32. * H1 + 9./32. * H2, R + 3./32. * R1 + 9./32. * R2, 3. * Hconf(a + 3./32. * a1 + 9./32. * a2, fourpiG, cosmo) * Hconf(a + 3./32. * a1 + 9./32. * a2, fourpiG, cosmo), params, fofR_type);
	...
	R4 = dtau * R_dot_RungeKutta(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, H + 1932./2197. * H1 - 7200./2197. * H2 + 7296./2197. * H3, R + 1932./2197. * R1 - 7200./2197. * R2 + 7296./2197. * R3, 3. * Hconf(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, fourpiG, cosmo) * Hconf(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, fourpiG, cosmo), params, fofR_type);
	...
	R5 = dtau * R_dot_RungeKutta(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, H + 439./216. * H1 - 8. * H2 + 3680./513. * H3 - 845./4104. * H4, R + 439./216. * R1 - 8. * R2 + 3680./513. * R3 - 845./4104. * R4, 3. * Hconf(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, fourpiG, cosmo) * Hconf(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, fourpiG, cosmo), params, fofR_type);
	...
	R6 = dtau * R_dot_RungeKutta(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, H - 8./27. * H1 + 2. * H2 - 3544./2565. * H3 + 1859./4104. * H4 - 11./40. * H5, R - 8./27. * R1 + 2. * R2 - 3544./2565. * R3 + 1859./4104. * R4 - 11./40. * R5, 3. * Hconf(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, fourpiG, cosmo) * Hconf(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, fourpiG, cosmo), params, fofR_type);
									 ...
									 const int fofR_type)
{
	...
	a1 = dtau * a_dot_RungeKutta(a, H);
	H1 = dtau * H_dot_RungeKutta(a, H, R);
	R1 = dtau * R_dot_RungeKutta(a, H, R, 3. * Hconf(a, fourpiG, cosmo, T00_hom_a3) * Hconf(a, fourpiG, cosmo, T00_hom_a3), params, fofR_type);

	a2 = dtau * a_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1);
	H2 = dtau * H_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1, R + 0.25 * R1);
	R2 = dtau * R_dot_RungeKutta(a + 0.25 * a1, H + 0.25 * H1, R + 0.25 * R1, 3. * Hconf(a + 0.25 * a1, fourpiG, cosmo, T00_hom_a3) * Hconf(a + 0.25 * a1, fourpiG, cosmo, T00_hom_a3), params, fofR_type);

	a3 = dtau * a_dot_RungeKutta(a + 3./32. * a1 + 9./32. * a2, H + 3./32. * H1 + 9./32. * H2);
	H3 = dtau * H_dot_RungeKutta(a + 3./32. * a1 + 9./32. * a2, H + 3./32. * H1 + 9./32. * H2, R + 3./32. * R1 + 9./32. * R2);
	R3 = dtau * R_dot_RungeKutta(a + 3./32. * a1 + 9./32. * a2, H + 3./32. * H1 + 9./32. * H2, R + 3./32. * R1 + 9./32. * R2, 3. * Hconf(a + 3./32. * a1 + 9./32. * a2, fourpiG, cosmo, T00_hom_a3) * Hconf(a + 3./32. * a1 + 9./32. * a2, fourpiG, cosmo, T00_hom_a3), params, fofR_type);

	a4 = dtau * a_dot_RungeKutta(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, H + 1932./2197. * H1 - 7200./2197. * H2 + 7296./2197. * H3);
	H4 = dtau * H_dot_RungeKutta(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, H + 1932./2197. * H1 - 7200./2197. * H2 + 7296./2197. * H3, R + 1932./2197. * R1 - 7200./2197. * R2 + 7296./2197. * R3);
	R4 = dtau * R_dot_RungeKutta(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, H + 1932./2197. * H1 - 7200./2197. * H2 + 7296./2197. * H3, R + 1932./2197. * R1 - 7200./2197. * R2 + 7296./2197. * R3, 3. * Hconf(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, fourpiG, cosmo, T00_hom_a3) * Hconf(a + 1932./2197. * a1 - 7200./2197. * a2 + 7296./2197. * a3, fourpiG, cosmo, T00_hom_a3), params, fofR_type);

	a5 = dtau * a_dot_RungeKutta(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, H + 439./216. * H1 - 8. * H2 + 3680./513. * H3 - 845./4104. * H4);
	H5 = dtau * H_dot_RungeKutta(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, H + 439./216. * H1 - 8. * H2 + 3680./513. * H3 - 845./4104. * H4, R + 439./216. * R1 - 8. * R2 + 3680./513. * R3 - 845./4104. * R4);
	R5 = dtau * R_dot_RungeKutta(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, H + 439./216. * H1 - 8. * H2 + 3680./513. * H3 - 845./4104. * H4, R + 439./216. * R1 - 8. * R2 + 3680./513. * R3 - 845./4104. * R4, 3. * Hconf(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, fourpiG, cosmo, T00_hom_a3) * Hconf(a + 439./216. * a1 - 8. * a2 + 3680./513. * a3 - 845./4104. * a4, fourpiG, cosmo, T00_hom_a3), params, fofR_type);

	a6 = dtau * a_dot_RungeKutta(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, H - 8./27. * H1 + 2. * H2 - 3544./2565. * H3 + 1859./4104. * H4 - 11./40. * H5);
	H6 = dtau * H_dot_RungeKutta(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, H - 8./27. * H1 + 2. * H2 - 3544./2565. * H3 + 1859./4104. * H4 - 11./40. * H5, R - 8./27. * R1 + 2. * R2 - 3544./2565. * R3 + 1859./4104. * R4 - 11./40. * R5);
	R6 = dtau * R_dot_RungeKutta(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, H - 8./27. * H1 + 2. * H2 - 3544./2565. * H3 + 1859./4104. * H4 - 11./40. * H5, R - 8./27. * R1 + 2. * R2 - 3544./2565. * R3 + 1859./4104. * R4 - 11./40. * R5, 3. * Hconf(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, fourpiG, cosmo, T00_hom_a3) * Hconf(a - 8./27. * a1 + 2. * a2 - 3544./2565. * a3 + 1859./4104. * a4 - 11./40. * a5, fourpiG, cosmo, T00_hom_a3), params, fofR_type);
									         ...
									         const int fofR_type)
{
	...
	Y1 = Y_dot_RungeKutta_trace(a, H, R, Y, Tbar(a, cosmo), fourpiG, cosmo, params, fofR_type);
	...
	Y2 = Y_dot_RungeKutta_trace(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau, Y + 0.5 * Y1 * dtau, Tbar(a + 0.5 * a1 * dtau, cosmo), fourpiG, cosmo, params, fofR_type);
	...
	Y3 = Y_dot_RungeKutta_trace(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau, Y + 0.5 * Y2 * dtau, Tbar(a + 0.5 * a2 * dtau, cosmo), fourpiG, cosmo, params, fofR_type);
	...
	Y4 = Y_dot_RungeKutta_trace(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau, Y + Y3 * dtau, Tbar(a + a3 * dtau, cosmo), fourpiG, cosmo, params, fofR_type);
									         ...
									         const int fofR_type)
{
	...
	H1 = H_dot_RungeKutta_trace(a, H, R);
	R1 = R_dot_RungeKutta_trace(Y);
	Y1 = Y_dot_RungeKutta_trace(a, H, R, Y, T_hom, fourpiG, cosmo, params, fofR_type);

	a2 = a_dot_RungeKutta_trace(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau);
	H2 = H_dot_RungeKutta_trace(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau);
	R2 = R_dot_RungeKutta_trace(Y + 0.5 * Y1 * dtau);
	Y2 = Y_dot_RungeKutta_trace(a + 0.5 * a1 * dtau, H + 0.5 * H1 * dtau, R + 0.5 * R1 * dtau, Y + 0.5 * Y1 * dtau, T_hom * Tbar(a + 0.5 * a1 * dtau, cosmo) / Tbar_0, fourpiG, cosmo, params, fofR_type);

	a3 = a_dot_RungeKutta_trace(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau);
	H3 = H_dot_RungeKutta_trace(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau);
	R3 = R_dot_RungeKutta_trace(Y + 0.5 * Y2 * dtau);
	Y3 = Y_dot_RungeKutta_trace(a + 0.5 * a2 * dtau, H + 0.5 * H2 * dtau, R + 0.5 * R2 * dtau, Y + 0.5 * Y2 * dtau, T_hom * Tbar(a + 0.5 * a2 * dtau, cosmo) / Tbar_0, fourpiG, cosmo, params, fofR_type);

	a4 = a_dot_RungeKutta_trace(a + a3 * dtau, H + H3 * dtau);
	H4 = H_dot_RungeKutta_trace(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau);
	R4 = R_dot_RungeKutta_trace(Y + Y3 * dtau);
	Y4 = Y_dot_RungeKutta_trace(a + a3 * dtau, H + H3 * dtau, R + R3 * dtau, Y + Y3 * dtau, T_hom * Tbar(a + a3 * dtau, cosmo) / Tbar_0, fourpiG, cosmo, params, fofR_type);
