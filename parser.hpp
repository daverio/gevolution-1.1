	...
	for(int i = 0; i < numparam; i++)
	{
		if(strcmp(params[i].name, pname) == 0)
		{
			...
			while ((comma = strchr(start, ',')) != NULL)
			{
				...
				else if(strcmp(item, "P") == 0 || strcmp(item, "p") == 0) pvalue |= MASK_P;
				...
				else if(strcmp(item, "multi-Gadget") == 0 || strcmp(item, "multi-Gadget2") == 0 || strcmp(item, "multi-gadget") == 0 || strcmp(item, "multi-gadget2") == 0) pvalue |= MASK_GADGET | MASK_MULTI;
				...
				else if(strcmp(item, "v") == 0 || strcmp(item, "velocity") == 0) pvalue |= MASK_VEL;
			...
			else if(strcmp(start, "P") == 0 || strcmp(start, "p") == 0) pvalue |= MASK_P;
			...
			else if(strcmp(start, "multi-Gadget") == 0 || strcmp(start, "multi-Gadget2") == 0 || strcmp(start, "multi-gadget") == 0 || strcmp(start, "multi-gadget2") == 0) pvalue |= MASK_GADGET | MASK_MULTI;
			...
			else if(strcmp(start, "v") == 0 || strcmp(start, "velocity") == 0) pvalue |= MASK_VEL;
...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, icsettings & ic)
{
	...
	{
		sim.modified_gravity_flag = MODIFIED_GRAVITY_FLAG_NONE;
	...
	if(sim.modified_gravity_flag == MODIFIED_GRAVITY_FLAG_FR)
	{
		...
		if(parseParameter(params, numparam, "f(R) model", par_string))
		{
			...
			{
				COUT << " Using f(R) with" << COLORTEXT_YELLOW << " EXPLICIT " << COLORTEXT_RESET << "Lambda term, Omega_Lambda = " << cosmo.Omega_Lambda << endl;;
			}
		}
		else
		{
