...
void writePowerSpectrum(Real * kbin, Real * power, Real * kscatter, Real * pscatter, int * occupation, const int numbins, const Real rescalek, const Real rescalep, const char * filename, const char * description, const double a, const int cycle = -1)
{
	if (parallel.isRoot())
	{
		...
		else
		{
			...
			if(cycle > -1) fprintf(outfile, "# cycle=%d\n", cycle);
